...
Geometry = "400x420+" + str(int(screen_width/2) - 200) + '+' + str(int(screen_height/2) - 200)
windowList = []
display_to_title = {}
saveList = {}
selected_app = "0"
    ...
    while True:
        ...
        if windowList != temp:
            try:
                ...
                # The UI has been destroyed; stop the background thread
                return
...
def load_settings():
    ...
    try:
        ...
        # First run: no settings file yet, use defaults silently
        return {"theme": "System", "apps": {}, "start_with_windows": False}
    except (json.JSONDecodeError, KeyError, TypeError, OSError) as e:
        # Corrupt or unreadable settings: warn instead of silently resetting
...
def safe_int(value, fallback=0):
    """Parse an int from untrusted settings/user input, with a fallback."""
    try:
...
def update_defaults(new_defaults):
    ...
    if new_defaults is None:
        ...
        settings["defaults"] = new_defaults
    save_settings(settings)

def change_save_defaults_event():
    global app_defaults
    if defaults_check_box.get() == 1:
        app_defaults = {
            "monitor": selected_monitor,
            "x_offset": custom_x_offset.get(),
            "y_offset": custom_y_offset.get(),
            "width": custom_width.get(),
            "height": custom_height.get(),
        }
        update_defaults(app_defaults)
    else:
        app_defaults = None
        update_defaults(None)

def reset_fields_for_new_app():
    """Prefill the entry fields for an app that has no saved config yet.
    Uses the saved defaults if the user enabled them, otherwise the
    selected monitor's native values."""
    global selected_monitor
    if app_defaults:
        default_monitor = app_defaults.get("monitor")
        if default_monitor in monitors:
            selected_monitor = default_monitor
            monitor_dropdown.set(default_monitor)
            label.configure(text="Display Resolution is " + str(monitors[default_monitor].width) + 'x' + str(monitors[default_monitor].height))
        mon = monitors[selected_monitor]
        custom_x_offset.set(str(safe_int(app_defaults.get("x_offset"), 0)))
        custom_y_offset.set(str(safe_int(app_defaults.get("y_offset"), 0)))
        custom_width.set(str(safe_int(app_defaults.get("width"), mon.width)))
        custom_height.set(str(safe_int(app_defaults.get("height"), mon.height)))
    else:
...
def make_borderless(app_name=None, write_needed=True):
    ...
    try:
        ...
        # Now position and resize the window (validate all values from UI/settings)
        if write_needed:
            mon = monitors[selected_monitor]
            target_resolution = (safe_int(custom_width.get(), mon.width), safe_int(custom_height.get(), mon.height))
            location_x = mon.x + safe_int(custom_x_offset.get(), 0)
            location_y = mon.y + safe_int(custom_y_offset.get(), 0)
        else:
            app_cfg = saveList[app_name]
            monitor_name = app_cfg.get("monitor")
            if monitor_name not in monitors:
                # Saved monitor no longer exists (unplugged / renamed): fall back
                monitor_name = selected_monitor
            mon = monitors[monitor_name]
            target_resolution = (safe_int(app_cfg.get("width"), mon.width), safe_int(app_cfg.get("height"), mon.height))
            location_x = mon.x + safe_int(app_cfg.get("x_offset"), 0)
            location_y = mon.y + safe_int(app_cfg.get("y_offset"), 0)

        if target_resolution[0] <= 0 or target_resolution[1] <= 0:
            print(f"Ignoring invalid target resolution {target_resolution} for {app_name}")
            return
...
def restore_window():
    ...
    if saved_key and saved_key in saveList:
        pre_win_height = safe_int(saveList[saved_key].get("pre_win_height"), 600)
        pre_win_width = safe_int(saveList[saved_key].get("pre_win_width"), 800)
...
def set_startup(startup):
    s_name = "NoMoreBorder"
    # Quote the path so Windows doesn't misparse paths containing spaces
    address = f'"{sys.executable}"'
    key = r"Software\Microsoft\Windows\CurrentVersion\Run"

    # Request only the access we actually need instead of KEY_ALL_ACCESS
...
app_defaults = current_settings.get("defaults")
...
defaults_check_box = ctk.CTkCheckBox(panel, text='Use current values as default for new apps', command=change_save_defaults_event)
defaults_check_box.grid(padx=5, pady=(8, 0))
defaults_check_box.select() if app_defaults else defaults_check_box.deselect()

# Prefill the fields from saved defaults on startup
if app_defaults:
    reset_fields_for_new_app()
